...
private struct MemoryLifecycleSection: View {
    ...
    var body: some View {
        ...
        ListGroup {
            ...
            if vm.prefillMemoryGuard {
                Row(
                    label: String(localized: "performance.memory.abort_under_pressure",
                                  defaultValue: "Abort Requests Under Pressure",
                                  comment: "Row label for abort-under-pressure toggle"),
                    sublabel: String(localized: "performance.memory.abort_under_pressure.sub",
                                     defaultValue: "Allow the memory guard to cancel in-flight requests under hard pressure. Turn off to avoid cancelling active work in normal operation; residual pressure is left to macOS compression and swap. A last-resort physical-memory safeguard may still stop a request to keep the process from crashing.",
                                     comment: "Sublabel for abort-under-pressure toggle")
                ) {
                    Toggle("", isOn: $vm.abortUnderPressure)
                        .labelsHidden().toggleStyle(.switch)
                }
            }
            Row(
...
private struct CacheSection: View {
    ...
    @Published private(set) var loadedAbortUnderPressure: Bool = true
    ...
    var hasPendingChanges: Bool {
        parsedMaxConcurrent != loadedMaxConcurrent
            ...
            || abortUnderPressure != loadedAbortUnderPressure
    ...
    func load(client: OMLXClient) async {
        do {
            ...
            if let mem = s.memory {
                ...
                self.abortUnderPressure = mem.abortUnderPressure ?? true
                self.loadedAbortUnderPressure = mem.abortUnderPressure ?? true
    ...
    func save(client: OMLXClient) async {
        ...
        if abortUnderPressure != loadedAbortUnderPressure {
            patch.memoryAbortUnderPressure = abortUnderPressure
        }
        ...
        do {
            ...
            self.loadedAbortUnderPressure = abortUnderPressure
            self.loadedMemoryGuardTier = tier
...
>>>>>>> 7ce1ac2 (feat(memory): add abort_under_pressure switch to suppress request cancellation)
